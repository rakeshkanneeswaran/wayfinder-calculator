  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-3 py-3">
        <h2 id="history-heading" className="text-xs tracking-wide text-zinc-500 uppercase">
          History
        </h2>
        <button
          type="button"
          onClick={onClear}
          disabled={empty}
          className="rounded px-2 py-1 text-xs text-zinc-500 hover:bg-zinc-100 hover:text-zinc-800 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-40"
        >
          Clear history
        </button>
      </div>
          ...
          {rows.map((row) => (
            <li key={row.id}>
              <button
                ...
                className="flex w-full flex-col items-end gap-0.5 px-3 py-2 text-right hover:bg-zinc-100 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none"
              >
                <span className="text-xs text-zinc-500">{row.expr}</span>
